...
2. 日历每日状态共4种：已完成、过期未完成、待完成、无任务，具体展示样式以UI设计稿为准
3. 月视图支持前后月份切换
4. 每日0点（北京时间）重置打卡状态
5. 同一天只能打卡一次
6. 显示最近参与的阅读计划，若有多个进行中的计划显示最近加入的一个
7. 任务卡片内直接展示当日所有任务项，需区分已完成和未完成状态
8. 点击"去完成"按钮跳转到F009 AI导语页
9. 任务过期规则：阅读计划进行中，往日未完成的任务仍可补做；阅读计划结束后，所有未完成的任务（含打卡、测评）不可再操作，状态标记为"已过期"
...
1. 任务列表按配置顺序逐步展示，包含阅读、导读单、测评、视频等任务类型。每次只展示当前待完成的任务，用户完成或跳过后才展示下一个；再次进入页面时，已完成的任务正常展示并可点击查看完成详情，从第一个未完成的任务继续
2. 任务项有三种状态：已完成（✓标记）、已跳过（序号不变+"已跳过"标记，灰色展示，不提供操作按钮）、待完成（序号+操作按钮）
3. 已完成的导读单/打卡任务支持"查看打卡详情"跳转
4. 待完成任务提供"跳过"和对应操作按钮，四种任务类型分别为：阅读任务显示"读完了"（线下阅读，无跳转）、导读单任务显示"去打卡"（跳转拍照打卡页）、测评任务显示"去测评"（跳转AI测评页）、视频任务显示"去观看"（跳转AI导读视频页）
5. 跳过操作不改变任务完成状态，仅当次跳过继续下一个任务；下次进入页面时，跳过的任务仍为未完成，从第一个未完成的任务重新开始
6. 导语气泡内嵌听书入口模块，显示当前阅读页码范围，点击"去听书"跳转阅读书籍页并自动开启听书播放器；若该书籍无听书或电子版资源，则不展示此模块
7. 每个任务项直接展示管理平台配置的导语内容（含任务参数），不单独显示任务标题
8. 任务导语中的任务关键词高亮显示，便于学生快速识别任务内容
...
| 任务列表     | List（列表）       | 展示当日所有任务项，区分已完成、已跳过和待完成三种状态                 |
...
| 成功横幅   | Card（卡片）   | 奖杯图标，"太棒了，打卡成功！"，"今日阅读任务已完成" |
| 奖励展示   | Text（文本）   | 获得积分数、连续打卡天数                             |
| 参考答案   | Card（卡片）   | 导读单参考答案图片，点击弹出图片查看器放大查看       |
| 返回导读页 | Button（按钮） | 主按钮，点击返回当前书籍的导读页                     |
...
排名数据从服务器获取，按阅读次数排名。阅读次数以完成当天所有任务（阅读+导读单）计为1次
